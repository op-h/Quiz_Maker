...
document.addEventListener('keydown',function(e){if(e.key==='F12'||(e.ctrlKey&&e.shiftKey&&'IJC'.includes(e.key)))e.preventDefault();});
  ...
  function getEmbeddedCss(lockCopyPaste, examMode, enableTimer) {
    return `:root{--bg-color:#050505;--panel-bg:rgba(10,15,10,0.6);--panel-hover:rgba(20,30,20,0.7);--border-color:rgba(60,80,60,0.5);--border-glow:rgba(159,239,0,0.6);--text-main:#b0c0b0;--text-muted:#6e7f6e;--text-bright:#ffffff;--accent-primary:#9fef00;--accent-primary-hover:#b4ff20;--accent-success:#39d353;--accent-success-hover:#2ea043;--accent-danger:#ff4444;--accent-warning:#e3b341;--htb-green:#9fef00;--font-main:'Inter',-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;--font-mono:'JetBrains Mono','Fira Code','Courier New',monospace;--shadow-md:0 4px 16px rgba(0,0,0,.9);--shadow-lg:0 16px 48px rgba(0,0,0,.95);--glow-blue:0 0 20px rgba(159,239,0,0.15);--radius-md:6px;--radius-lg:10px;--input-bg:rgba(5,10,5,0.8);--sidebar-bg:rgba(5,10,5,0.7);--workspace-bg:transparent;--level-btn-hover:rgba(159,239,0,0.1)}
...
body{background:var(--bg-color);background-image:linear-gradient(rgba(159,239,0,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(159,239,0,0.04) 1px,transparent 1px);background-size:40px 40px;color:var(--text-main);font-family:var(--font-main);display:flex;flex-direction:column;min-height:100vh;height:100vh;overflow:hidden;user-select:${lockCopyPaste ? 'none' : 'auto'};-webkit-font-smoothing:antialiased;transition:background .2s,color .2s}
.modal-box,.challenge-card,.sidebar,.top-bar{backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);}
